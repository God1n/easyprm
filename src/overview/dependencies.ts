...
export function renderDependencies(
  ...
  const phaseById = new Map(phases.map((p) => [p.frontmatter.id, p]));
...
`;
  ...
  for (const [phaseId, phaseTasks] of phaseGroups) {
    const title = phaseById.get(phaseId)?.frontmatter.title ?? phaseId;
    lines.push(`  subgraph ${phaseId} ["${phaseId}: ${title}"]`);
